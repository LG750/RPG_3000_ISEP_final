...
public class Game {
    ...
    public Game(InputParser inputParser) {
        ...
        displayMessage("RPG 3000 ISEP - Léonard GRANGE.  Bienvenue dans le meilleur des RPG! \n \n");

        // Liste contenant nos différents héros
        heros = new ArrayList<>();

        displayMessage("Veuillez tout d'abord indiquer le nombre de héros souhaité, entre 1 et 5 héros : ");
        ...
        while (!validNumber) {
            try {
                ...
                while (numberHero > 5 || numberHero <= 0) {
                    System.out.println("Veuillez à respecter les règles du jeu, s'il-vous-plait... (min : 1 héros, max : 5 héros)");
                    numberHero = scanner.nextInt();
                }
                validNumber = true;
            } catch (InputMismatchException e) {
                System.out.println("Veuillez saisir un numéro valide ! s'il-vous-plait");
                scanner.nextLine();
            }
        }
        ...
        for (int i = 0; i < numberHero; i++) {
            ...
            if (i == 0) {
                displayMessage("Veuillez choisir la classe de votre premier héros, svp : ");
            } else {
                displayMessage("\nChoississez svp la classe de votre " + (i + 1) + "-ème héros, svp : ");
            }
            displayMessage("[1] Warrior: 6 hp \n[2] Hunter: 8 hp \n[3] Mage: 7 hp \n[4] Healer: 10 hp");
            ...
            while (!tmp) {
                try {
                    ...
                    while (firstHeroNumber > 4 || firstHeroNumber <= 0) {
                        displayMessage("Veuillez saisir un numéro valide, s'il-vous-plait!");
                        firstHeroNumber = scanner.nextInt();
                    }
                    tmp = true;
                } catch (InputMismatchException e) {
                    displayMessage("Veuillez saisir un numéro valide, svp! Choisissez la classe de votre " + (i + 1) + "-ème héros : (entrez le numéro correspondant)?");
                    scanner.nextLine();
                }
            }


            displayMessage("Choississez un nom pour votre héros, svp ?");
            ...
            switch (firstHeroNumber) {
                ...
                case 1:
                    displayMessage("Vous avez le choix entre deux armes : [1] Knife: 3 points de dégâts ou [2] AmericanPunch: 2 points de dégâts. Veuillez entrer le numéro de votre arme : ");
                    boolean tmp1 = false;
                    while (!tmp1) {
                        try {
                            ...
                            displayMessage("Vous devez saisir un numéro valide ! [1] Knife: 3 points de dégâts ou [2] AmericanPunch: 2 points de dégâts. Veuillez entrer le numéro de votre arme : ");
                            scanner.nextLine();
                        }
                    }
                ...
                case 2:
                    displayMessage("Vous avez le choix entre deux armes : [1] Arbalète: 2 points de dégâts ou [2] Arc: 2 points de dégâts. Veuillez entrer le numéro de votre arme : ");
                    boolean tmp2 = false;
                    while (!tmp2) {
                        try {
                            firstHeroWeapon = scanner.nextInt();
                            ...
                            displayMessage("Vous devez saisir un numéro valide ! [1] Arbalète: 2 points de dégâts ou [2] Arc: 2 points de dégâts. Veuillez entrer le numéro de votre arme : ");
                            scanner.nextLine();
                        }
                    }
                ...
                case 3:
                    displayMessage("Vous avez le choix entre deux armes : [1] Boule de feu: 2 points de dégâts ou [2] Stone: 3 points de dégâts. Veuillez entrer le numéro de votre arme : ");
                    boolean tmp3 = false;
                    while (!tmp3) {
                        try {
                            firstHeroWeapon = scanner.nextInt();
                            while (firstHeroWeapon > 2 || firstHeroWeapon <= 0) {
                                displayMessage("ATTENTION : Veuillez saisir un numéro valide, svp!");
                                firstHeroWeapon = scanner.nextInt();
                            }
                            tmp3 = true;
                        } catch (InputMismatchException e) {
                            displayMessage("Vous devez saisir un numéro valide! [1] Boule de feu: 2 points de dégâts ou [2] Stone: 3 points de dégâts. Veuillez entrer le numéro de votre arme : ");
                            scanner.nextLine();
                        }
                    }
        ...
        enemies.add(new Evil("Evil"));
        enemies.add(new Wizard("Wizard"));
        enemies.add(new Troll("Troll"));
        Collections.shuffle(enemies);         // Mélange de la liste pour que l'ordre des ennemies soit aléatoire
        enemies.add(new BigEnnemy("BigEnnemy"));
    }
    ...
    public void start() {
        ...
        // Chaque héros possède de la nourriture par défaut. Possibilité d'augmenter l'efficacité de la nourriture
        // si l'équipe gagne un combat.
        Food lemba_warrior = new Food("lemba",3);
        Food lemba_hunter = new Food("lemba",3);
        Food lemba_mage = new Food("lemba",3);
        Food lemba_healer = new Food("lemba",3);
        for(Hero c:heros){
        ...
        while (true) {
            ...
            //Génération d'un nombre aléatoire entre 0 et 1 pour savoir qui des héros/ennemis attaque
            Random r = new Random();
                ...
                while (!tmp4) {
                    try {
                        ...
                        while (ixHero > heros.toArray().length || ixHero <= 0) {
                            System.out.println("Veuillez saisir une position valide, svp");
                            ixHero = scanner.nextInt();
                        }
                        tmp4 = true;
                ...
                case 1:
                    ...
                    if (goodOne.getHealthPoint() <= 0) {
                        ...
                        if(goodOne instanceof Healer){
                            ...
                            if(notHealerList.size() == 0) displayMessage("Il n'y a pas de héros à soigner : les Healer sont vulnérables!");
                                ...
                                switch (choix){
                                    ...
                                    case 2:
                                        displayMessage("Votre Healer " + goodOne + " boît une potion... Sa Mana est restaurée de 2 points !");
                                        ((SpellCaster) goodOne).setMana((((SpellCaster) goodOne).getMana())+2);
                                        displayMessage("\t" + goodOne + " possède désormais : " + ((SpellCaster) goodOne).getMana() + " points de Mana\n");
                                        break;
                                }
                        ...
                        // Sinon : riposte d'un des héros
                            ...
                            while(!tmp) {
                                try {
                                    ...
                                    displayMessage("Exception " + e + ". Saisir un numéro valide, svp!");
                                    scanner.nextLine();
                                }
                            }
                            ...
                            switch (choice){
                                ...
                                case 3:
                                    displayMessage("Votre SpellCaster " + goodOne + " boît une potion... Sa Mana est restaurée de 2 points !");
                        ...
                        if (badOne.getHealthPoint() <= 0) {
                            ...
                            // Système de récompense : augmenter points de vie ou points de dégâts ou nombre de flèches/mana
                            if(enemies.size() != 0)
                                recompense(heros);
                        }
                        }
                        ...
                        if(notHealerList.size() == 0) displayMessage("Il n'y a pas de héros à soigner : les Healer sont vulnérables!");
                            ...
                            while(!tmp) {
                                try {
                                    ...
                                    displayMessage("Exception" + e + ". Veuillez entrer un numéro valide, svp!");
                                    scanner.nextLine();
                                }
                            }
    ...
    public static void displayStatus(List<Hero> h, List<Combatant> e) {
        ...
        System.out.println();
        System.out.print(ANSI_MAGENTA + "ENNEMIS : " + ANSI_RESET );
        for (Combatant c: e) {
            System.out.print(ANSI_MAGENTA + c.getName() + "(" + c.getHealthPoint() + " hp) " + ANSI_RESET);
        }
        System.out.println("\n###############################################################################\n");
    }
            ...
            case 5:
                ...
                while(!tmp){
                    try{
                        ...
                        displayMessage("Veuillez saisir un nombre valide svp ...");
                        sc.nextLine();
                    }
                }
                break;
